...
def create_dataloaders(x, y, split_ratio, batch_size):
    ...
    train_loader = DataLoader(train_dataset,batch_size=train_size, shuffle=True, drop_last=True)
...
def training_model(x,y,data_size, learning_rate, batch_size): 
    ...
    print(f"training with {n_batches} batches in {train_size} points of data")
    while dloss > 1e-3 and nepoch<5000:
        for batch_idx, (x_train_slice, y_train_slice) in enumerate(train_loader):
            ...
            if nepoch % 50 == 0:
                ...
                print(f"The loss function for {nepoch} is {new_loss}")
                dloss=np.abs(new_loss-previous_loss)
...
df.to_csv('SGD_test_loss.csv')

df = pd.DataFrame(train_loss, 
                  columns=['1e-05', '0.0001', '0.001', '0.01', '0.1'],  # Example learning rates
                  index=[0.3, 0.4, 0.5, 0.6, 0.7, 0.8])  # Example data sizes
df.to_csv('SGD_train_loss.csv')

df = pd.DataFrame(nepochs, 
                  columns=['1e-05', '0.0001', '0.001', '0.01', '0.1'],  # Example learning rates
                  index=[0.3, 0.4, 0.5, 0.6, 0.7, 0.8])  # Example data sizes
df.to_csv('SGD_nepochs.csv')

df = pd.DataFrame(has_finished_mat, 
                  columns=['1e-05', '0.0001', '0.001', '0.01', '0.1'],  # Example learning rates
                  index=[0.3, 0.4, 0.5, 0.6, 0.7, 0.8])  # Example data sizes
df.to_csv('SGD_has_finished.csv')
